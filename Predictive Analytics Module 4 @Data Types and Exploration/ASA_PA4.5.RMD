...
levels(data.mortality$prodcat)

data.mortality.wholeLife <- data.mortality[data.mortality$prodcat == "WL",]
data.mortality.term <- data.mortality[data.mortality$prodcat == "TRM",]
head(data.mortality.wholeLife)
head(data.mortality.term)

summary(data.mortality.wholeLife$issage)
summary(data.mortality.term$issage)
```
...
stats.spread.wholeLife <- data.frame(variance = var(data.mortality.wholeLife$issage),
                           standardDev = sd(data.mortality.wholeLife$issage),
                           iqr = IQR(data.mortality.wholeLife$issage),
                           row.names = c("issage"))
stats.spread.wholeLife

stats.spread.term <- data.frame(variance = var(data.mortality.term$issage),
                           standardDev = sd(data.mortality.term$issage),
                           iqr = IQR(data.mortality.term$issage),
                           row.names = c("issage"))
stats.spread.term

###################################
stats.spread.function <- function(col.name) {
  row.names = c(col.name)
  
  df <- data.frame(variance = var(data.mortality[, row.names]),
                   standardDev = sd(data.mortality[, row.names]),
                   iqr = IQR(data.mortality[, row.names]),
                   row.names = row.names)
  return(df)
}

stats.spread.function("issage")
stats.spread.function("duration")
stats.spread.function("exposure_cnt")
stats.spread.function("exposure_face")
...
#The table function is extrememly useful. Use ?table to learn more about it.
...
p2 <- ggplot(data = data.mortality, aes(x = prodcat, y = exposure_cnt)) +
  geom_bar(stat = "summary", fun.y = sum, fill = c(1:4), alpha = 0.5)

# Summary by face amount exposure variable
p3 <- ggplot(data = data.mortality, aes(x = prodcat, y = exposure_face)) +
  geom_bar(stat = "summary", fun.y = sum, fill = c(1:4), alpha = 0.5)
