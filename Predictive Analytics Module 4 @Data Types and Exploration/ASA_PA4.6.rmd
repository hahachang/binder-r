...
```{r}
#CHUNK 4
p1 <- ggplot(data = subset(data.mortality, actual_cnt >= 1), 
             aes(x = resind_ind, 
                 y = actual_face, 
                 fill = resind_ind)) +
        geom_boxplot() +
        scale_y_log10()
p2 <- ggplot(data = subset(data.mortality, actual_cnt >= 1), 
             aes(x = uwkey, 
                 y = actual_face, 
                 fill = uwkey)) +
        geom_boxplot() +
...
p3 <- ggplot(data = subset(data.mortality, actual_cnt >= 1), 
             aes(x = sex, 
                 y = actual_face, 
                 fill = sex)) +
        geom_boxplot() +
        scale_y_log10()
p4 <- ggplot(data = subset(data.mortality, actual_cnt >= 1), 
             aes(x = smoker, 
                 y = actual_face, 
                 fill = smoker)) +
        geom_boxplot() +
