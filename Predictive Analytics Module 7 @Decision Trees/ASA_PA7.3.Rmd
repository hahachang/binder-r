...
We'll fit the random forest model manually using the randomForest package.
...
# Set the target (could use ifelse as well)
data.mortality$target[data.mortality$actual_cnt == 0] = 0
data.mortality$target[data.mortality$actual_cnt >= 1] = 1
# This is to force the Random Forest to recognize a classification problem
...
# Look at the first six predictions. The indices are not 1, 2, 3, ... because they represent the original rows that were selected for the testing set.
...
CHUNK 7 tries to fix that problem by changing the cutoff for predicting a 1. Recause random forest models do not directly provide a probability for each prediction, the caret package uses the out of bag error (see Introduction to Statistical Learning for more on this concept). 
...
#Use the cutoff of 0.9 to make a preduction. 
...
#CHUNK 9
set.seed(1000)
data.training.us <- rbind(data.training[data.training$target == 1,], # Keep all of the positive observations
                          data.training[data.training$target == 0,][sample.int(nrow(data.training[data.training$target == 0,]),
                                                                               size = nrow(data.training[data.training$target == 1,])),]) # Take a sample of the negative observations with the same size as the number of positive observations
...
# Note that we are still using the original test data. Why don't we need to undersample the test data?
predictions <- predict(model.rf, data.testing)
summary(predictions)
confusionMatrix(predictions,data.testing$target)
```
...
indices <- c(1:nrow(data.training), # Keep all of the original training data
             rep(which(data.training$target == 1), 50)) # Get duplicates of the positive variables (we have chosen 50 to roughly balance the classes)
...
In our example, we will use the caret package to tune the number of features (mtry) parameter of the random forest model usign repeated cross validation. Note that the caret package only supports tuning of mtry parameter of a random forest model. 
...
```{r}
#CHUNK 14
rfGrid <- expand.grid(mtry = c(1,3,5,7)) # the numer of features to select from for each split
```
...
sampling: "down" is undersampling. Other methods include "up" (oversampling), "SMOTE" and "ROSE" (hybrid methods).

```{r}
#CHUNK 15

# final accuracy = average(round1, round2,...round15)
ctrl <- trainControl(method = "repeatedcv", 
                     number = 5, # do 5-fold cross validation
                     repeats = 3, # and repeated 3 times for robustness
                     sampling = "down") # undersampling
```

Run CHUNK 16 to train the model. This might take a while. Note that at your exam your data file will not have 350,000 observations and thus complex models can be run without this type of delay.
...
model.rf.tuned <- train(target ~.,
                        ...
                        ntree = 50, # the default is 500, setting to 50 will save us a lot of computation time
                        importance = TRUE)
...
for each issue age, a set of predictions is obrained using the actual observed values for the other variable. Then, these predictions are averaged. Assuming that the observed values of the other variables are representative of the population of future observed values, we have estimated the predicted value for a randomly selected individual with that issue age. Note that for classification problems, the predcitions are on a logit scale log(p/(1-p)) and so a value of zero implies that for that issue age, the average prediction is 50% Yes and 50% No.

```{r}
#CHUNK 20
library(pdp)

partial(model.rf.tuned, 
        train = data.training, 
        pred.var = "issage", 
        plot= TRUE, 
        rug = TRUE, 
...
The following CHUNKs work through the example from Section 23.6 of R for Everyone. Run CHUNK 21 to load the data and fit and evaluate a single tree. The initial code is the same as what was presented earlier but with the predictor variables changed to match the example. There is extra code for evaluation.
...
credit.rf <- randomForest(formula = Credit ~ CreditHistory + Purpose + Employment + Duration + Employment + Age, 
                          data = credit, 
                          ntree = 500,
...
Run CHUNK 26 to fit a model using two straight lines. This is done manually be creating a function that calcuates the sum of squared errors and then optimizing it. You will not be expected to do this type of manual fitting at your exam.
...
Using xgboost has some data preparation challenges. We will switch to caret because it is much easier to set up and run (though less flexible). While xgboost will be available at your exam, there is no expectation that you will use it.
...
data.training.mf  <- model.frame(as.formula(paste("~",paste(vars,collapse = "+"))),
                                 data = head(data.training))

# A model (or design) matrix only contains numerical values. Factors are dummy coded by default
data.training.mm  <- model.matrix(attr(data.training.mf, "terms"),
                                  data = data.training)

# A XGB dense matrix contains an R matrix and metadata [optional]
data.training.dm  <- xgb.DMatrix(data.training.mm,
                                 label = data.training$target, 
                                 missing = -1)

data.testing.mf  <- model.frame(as.formula(paste("~",paste(vars,collapse = "+"))),
                                data = head(data.testing))
data.testing.mm  <- model.matrix(attr(data.testing.mf,"terms"),
                                 data = data.testing)
data.testing.dm  <- xgb.DMatrix(data.testing.mm,
                                label = data.testing$target, 
                                missing = -1)
...
Note that we haven't needed to balance the data this time to compensate for our low response rate. This is due to the improved performance of the GBM compared with the random forest and its property of focusing on the data points it is predicting poorly.

---
Run CHUNK 36 to set up the data for parameter tuning.

Because we are using caret, we don't need to worry about creating the special xgboost data structure ourselves.
...
Run CHUNK 37 to set up the grid. Note that some parameters are not allowed to vary, but we still want to specify the value to use.

```{r}
#CHUNK 37

xgbGrid <- expand.grid(max_depth = c(1, 3, 7),
                       nrounds = 700, # The number of trees/iterations
                       eta = c(0.01, 0.1), # learning rate
                       colsample_bytree = c(0.6, 0.9), # proportion of features used in each tree
                       gamma = 0,
                       min_child_weight = 1,
                       subsample = 0.6 # proportion of observations used in each tree
                       ) 
...
Note that whithin caret, we can't pass in all of the parameters we used previously for xgboost. More specifically, we cannot use a binary logistic objective. We will use down-sampling here to deal with our imbalanced data/low response rate (this will also reduce the training time).

---
summaryFunction	
a function to compute performance metrics across resamples. The arguments to the function should be the same as those in defaultSummary. Note that it method = "oob" is used, this option is ignored and a warning is issued.
...
ctrl <- trainControl(method = "cv", 
                     number = 2, # In the interest of computation time, we will do 2-fold cross validation
                     summaryFunction = twoClassSummary, # computes sensitivity, specifity and area under ROC curve
                     classProbs = TRUE,
                     sampling = "down" # initiate down sampling, this underweights dominant class
                     ) 
...
The caret model has limitations in that it doesn't implement the full functionality of the xgboost model (allowing us to specify chosen objective functions and evaluation metrics, including early stopping). We can, however, implement the parameter tuning process manually (rerunning xgboost using different parameters).
...
Run CHUNK 40 to evaluate variable importance. Note: This implementation of variable importance defaults to the top category given a value of 100 and all others than scaled.
...
Task 10b is to place the first 200 observations in the training set and the reamining observations in the testing set. This is genarally not good practice unless you are absolutely certain the rows are random observations from the population. Otherwise, it is better to randomize the selection as done in all the exmaples in this module.
...
model.Hitters.rf <- randomForest(formula = Salary ~ ., 
                         data = data.Hitters.train)
predict.Hitters.rf <- as.numeric(predict(model.Hitters.rf, newdata = data.Hitters.test))
SSE.predict.Hitters.rf <- sum((predict.Hitters.rf - data.Hitters.test$Salary)^2)/nrow(data.Hitters.test)
SSE.predict.Hitters.rf
```

For further practice consider adding cross validation with 10 folds.

Task E10d is to use a boosted model based on regression trees. To allow parameter tuning, use caret here, again with default parameters. CHUNK 37 may provide a model. Again determine the SSE for the test set and see if this model did better. You will note that if you do not set a tuning grid the pacakge creates its own grid. You can save time by making your own (smaller) grid.
...
SSE.predict.Hitters.xgb <- sum((predict.Hitters.xgb - data.Hitters.test$Salary)^2)/nrow(data.Hitters.test)
SSE.predict.Hitters.xgb

```

Task E10e is to obtain variable improtance from this model. It is done for you here:
...
Task E10f is to perform linear regression (OLS) uwsing all variables as an alternative to a regression tree. Perforem the regression and check the SSE.
...
SSE.predict.Hitters.ols <- sum((predict.Hitters.ols - data.Hitters.test$Salary)^2)/nrow(data.Hitters.test)
SSE.predict.Hitters.ols
...
SSE.predict.Hitters.glm <- sum((predict.Hitters.glm - data.Hitters.test$Salary)^2)/nrow(data.Hitters.test)
SSE.predict.Hitters.glm
...
CHUNK E11a loads the data, changes the target variables to 1 and 0, and creates that training and testing sets.
...
max_depth from 1 to 7, nrounds to 500, eta (or learning rate) from .01 to .05 by .01, colsample_bytree to .5 and .8, gamma to zero, min_child_rate to 1 and subsample to .6. 
...
xgbGrid <- expand.grid(max_depth=c(1:7),
                       nrounds=500,
                       eta = c(.01, .05, .01),
...
ctrl <- trainControl(method="cv", number=4, 
                     classProbs = TRUE,
                     sampling = c("down", "up")) # sample unbalanced sample up and down
...
varImp(xgb.tuned) # get variable importances
...
Lets try a different method, build a Random Forest model on the caravan dataset with caret with the following model specifications:  
...
#Number of variables randomly sampled as candidates at each split. Note that the default values are different for classification (sqrt(p) where p is number of variables and regression (p/3)
...
# if you care to, here is how you can adjust the threshold for caret confustion matrix
