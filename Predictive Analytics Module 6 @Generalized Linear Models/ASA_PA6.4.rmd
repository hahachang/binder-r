---
title: "Predictive Analtyics Exam Module 6 Section 4"
...
# The parameter estimates are significant. This will be further discussed later in the module. For now just note that all variables are highly sigficant (p-values extremely small).
...
#The summary shows that the gender:age interaction is signficant.
summary(glm.interact)

# Or calculate the change in AIC between the two models (with and without the interaction). This again indicates the interactions has improved the model (more on AIC later).
...
CHUN 4 loads the Insurance data and displays the first few values.
...
mod1 <- glm(Claims ~ District + Group + Age, 
            data = Insurance,
            family = poisson)
mod1$coefficients
...
CHUNK 6 fits several more models to illustate the use of weights

```{r}
# CHUNK 6
mod3 <- glm(Claims ~ District + Group + Age, 
            data = Insurance,
            family = gaussian)
mod1$coefficients
...
mod4 <- glm(AvgClaims ~ District + Group + Age, 
            data = Insurance, 
            family = gaussian)
mod4$coefficients
pred4 <- predict(mod4,type="response")

mod5 <- glm(AvgClaims ~ District + Group + Age, 
            data = Insurance, 
            weights = Holders, 
            family = gaussian)
mod5$coefficients
