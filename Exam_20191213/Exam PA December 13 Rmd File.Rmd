---
title: "December 13, 2019 Exam PA Rmd file"
...
df$occupation_comb <- mapvalues(df$occupation, var.levels, c("Group12", "Group12", "Group123", "Group123", "Group123", "GroupNA"))
...
These chunks create a boosted tree. Do not change any of the parameters.

```{r}
data.train$value_flag <- as.factor(ifelse(data.train$value_flag == 1, "High", "Low")) 
# xgboost requires factors for the target, will be set back to 0,1 at the end of this analysis.
...
model.xgb.tuned <- train(value_flag ~ VARIABLES,
  data = data.train,
  method = "xgbTree",
  eval_metric = "error",
  trControl = ctrl,
...
Construct and evaluate a GLM using regularized regression. 

The following code runs a GLM using elasticnet and cross validation to set lambda. Replace VARIABLES with the predictor variables to be used.
...
X <- model.matrix(value_flag ~ VARIABLES, data.train)

m <- cv.glmnet(
  x = X,
  y = data.train$value_flag,
  family = "FAMILY", # replace with selected family name
  type.measure = "class",
  alpha = 1
) 
# alpha = 1 implies LASSO, alpha = 0 implies ridge, values between 0 and 1 imply elasticnet
plot(m)
```
...
m.best <- glmnet(
  x = X,
  y = data.train$value_flag,
  family = "FAMILY", lambda = m$lambda.min,
  alpha = 1
)
X.test <- model.matrix(value_flag ~ VARIABLES, data.test)
m.best$a0 # Intercept
